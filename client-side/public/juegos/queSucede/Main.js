...
class Main extends React.Component {
	...
	generarEjercicios() {
		...
		for (var i = 0; i < this.numeroPreguntas; i++) {
			...
			indices.push(index);
			if (this.state.dificultad == "Facil" ) this.ejercicios.push(this.props.config["niveles"][0][index]);
			else if (this.state.dificultad == "Mediano") this.ejercicios.push(this.props.config["niveles"][1][index]);
			else this.ejercicios.push(this.props.config["niveles"][2][index]);
		}
	}

	siguiente() {
		if (this.state.index == null) this.props.addToast("¡Usted no ha seleccionado una respuesta!");
		else {
			this.setState({
			...
			if(this.ejercicios[this.state.pregunta]["respuesta"][0] == this.ejercicios[this.state.pregunta]['opciones'][this.state.index]){
			...
			return (
				<div>
					<div className="offset-12 text-center">
						<Img
							url={'/juegos/'+this.props.filename+'/data/img/' + this.state.dificultad + "/" + carpeta + "/escena.png"}
							seleccionar={()=>{}}
							/>
					</div>
					<div className="col">
